...
var DEFAULT_ADMIN_USERNAME = process.env.ADMIN_USERNAME || "kjoshi2064@gmail.com";
var DEFAULT_ADMIN_PASSWORD_RAW = process.env.ADMIN_PASSWORD || "kishor99farewell";
...
async function getAdminCredentials() {
  try {
    const credRef = (0, import_firestore.doc)(db, "admin_credentials", "current");
    const credSnap = await (0, import_firestore.getDoc)(credRef);
    if (credSnap.exists()) {
      return credSnap.data();
    } else {
      const defaultHash = import_bcryptjs.default.hashSync(DEFAULT_ADMIN_PASSWORD_RAW, 10);
      const defaultData = {
        username: DEFAULT_ADMIN_USERNAME.toLowerCase(),
        password_hash: defaultHash,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      };
      await (0, import_firestore.setDoc)(credRef, defaultData);
      return defaultData;
    }
  } catch (err) {
    console.error("Error reading admin credentials from Firestore, using default:", err);
    return {
      username: DEFAULT_ADMIN_USERNAME.toLowerCase(),
      password_hash: import_bcryptjs.default.hashSync(DEFAULT_ADMIN_PASSWORD_RAW, 10)
    };
  }
}
...
app.post("/api/admin/login", async (req, res) => {
  try {
    ...
    const isMatchUsername = inputUsername === credentials.username.toLowerCase() || inputUsername === DEFAULT_ADMIN_USERNAME.toLowerCase() || inputUsername === "kjoshi2064@gmail.com" || inputUsername === "admin";
    if (!isMatchUsername || !import_bcryptjs.default.compareSync(password, credentials.password_hash)) {
...
app.post("/api/admin/change-password", authenticateToken, async (req, res) => {
  try {
    ...
    const credentials = await getAdminCredentials();
    if (currentPassword && !import_bcryptjs.default.compareSync(currentPassword, credentials.password_hash)) {
      return res.status(401).json({ error: "Current password does not match." });
    }
    const updatedUsername = newUsername && newUsername.trim() ? newUsername.trim().toLowerCase() : credentials.username;
    ...
    await (0, import_firestore.setDoc)(credRef, updatedData);
    return res.json({
      success: true,
      message: "Admin credentials updated and saved permanently in Firestore! \u2764\uFE0F",
      username: updatedUsername
    });
  } catch (error) {
